...
def setup_anndata(
    copy_number_adata: ad.AnnData,
    scamp_tsv: str,
    temp_folder: str,
    cn_threshold: float,
    cn_percentile_threshold: float,
    umap_name: str,
    ...
    """
    cells = copy_number_adata.obs_names.intersection(expression_adata.obs_names)
    copy_number_adata = copy_number_adata[cells].copy()
    ...
    for index, row in scamp.iterrows():
        ...
        if row["pred"]:
            new_row = {
                "gene_set_name": "All ecDNA Genes",
                "gene_set_description": "Predicted ecDNA by scAmp",
                "gene_symbol": gene,
                "gene_description": "",
            }
            ...
            if "cluster" in row:
                new_row = {
                    "gene_set_name": f"ecDNA Cluster {row['cluster']}",
                    "gene_set_description": (f"Predicted ecDNA by scAmp, cluster {row['cluster']}"),
                    "gene_symbol": gene,
                    "gene_description": "",
                }
                gene_set_df.loc[len(gene_set_df)] = new_row
    ...
    # Add cell sets
    add_cell_sets(copy_number_adata, gene_set_df, cn_threshold, cn_percentile_threshold)
...
def add_cell_sets(
    ...
    for i, gene in enumerate(gene_list):
        ...
        adata.obs = adata.obs[
            [f"{gene}_ecDNA"] + [c for c in adata.obs.columns if c != f"{gene}_ecDNA"]
        ]
    adata.obs = adata.obs[
        ["Number of ecDNA Positive Genes"]
        + [c for c in adata.obs.columns if c != "Number of ecDNA Positive Genes"]
    ]
...
def setup_expression(
    ...
    Returns:
        An anndata object with expression data for genes present in cn_adata.
    """
...
def setup_copynumber(copy_number_file: str) -> ad.AnnData:
    ...
    Returns:
        An anndata object with copy-number data.
    """
    # Detect file extension
    ...
    var = pd.DataFrame({"idx": range(1, counts_df.shape[1] + 1)}, index=counts_df.columns)
...
def get_umap(umap_name: str, adata: ad.AnnData, temp_folder: str) -> None:
